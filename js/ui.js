const ui = {
    ...
    updateSummary: (transactions) => {
        ...
        ui.totalIncomeEl.innerHTML = `<i class="fas fa-arrow-up icon"></i> ${income.toFixed(2)}`;
        ui.totalExpenseEl.innerHTML = `<i class="fas fa-arrow-down icon"></i> ${expense.toFixed(2)}`;
        ui.balanceEl.innerHTML = `<i class="fas fa-wallet icon"></i> ${balance.toFixed(2)}`;
    },
    ...
    createTransactionRow: (transaction) => {
        ...
        const statusIcon = transaction.type === 'income' ? 'fa-circle-up' : 'fa-circle-down';
        row.innerHTML = `
            <td class="status-cell ${transaction.type}-color" data-label="Status"><i class="fa-solid ${statusIcon}"></i></td>
            <td data-label="Title">${transaction.title}</td>
            <td data-label="Category">${transaction.category}</td>
            <td data-label="Amount" class="amount-cell ${transaction.type}-color">${transaction.type === 'income' ? '+' : '-'}${transaction.amount.toFixed(2)}</td>
            <td data-label="Action"><button class="delete-btn" onclick="app.deleteTransaction(${transaction.id})"><i class="fas fa-trash-alt"></i></button></td>
        `;
        return row;
    },
